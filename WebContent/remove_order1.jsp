...
<%}%>
    ...
    <div class="login_box">
		<h1 class="title">删除订单</h1>
			
<%    
ResultSet rs;
int oid = Integer.parseInt(request.getParameter("order_id"));
String Sql="select  * from purchase_order where id='"+oid+
"' and commissioned_employee_id ='"+u.getId()+"'";
//System.out.println(Sql);
rs = workm.executeQuery(Sql);
if(!rs.next()){
	session.setAttribute("error_message","您不能操作未存在或非您申请创建的订单");
	%>
...
%>
<form method="post" id="mkforid"  action="remove_order2.jsp" >
   ...
   <table>
      <tr>
       ...
       <%  if(rs.getString("state").equals("unsubmit"))
		{
		%>
      <input class="button_box" class="button_box" type="submit" value="确认删除">
       <%
		}
		else
		{
